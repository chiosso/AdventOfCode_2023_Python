...
class Hand:
    ...
    def getHandType(self):
        # If there are any jokers, find the best way to use them
        if 'J' in self.cards.keys():
            biggestOtherCardGroupSize = 0
            biggestOtherCardGroup = None
            for c in self.cards.keys():
                if c != 'J' and self.cards[c] > biggestOtherCardGroupSize:
                    biggestOtherCardGroupSize = self.cards[c]
                    biggestOtherCardGroup = c
            if biggestOtherCardGroup != None:
                self.cards[biggestOtherCardGroup] = self.cards[biggestOtherCardGroup] + self.cards['J']
                self.cards.pop('J')

        # Perform standard logic now that Jokers have been handled
...
def generateSortStringForHand(hand):
    return hand.cardsString.replace('A','E').replace('K','D').replace('Q','C').replace('J','1').replace('T','A')
